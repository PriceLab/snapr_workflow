...
do
    ...
    ls -la /resources/assemblies/ref-genome.fa | awk '{print $3}' | sort > /tmp/fs-output$PID
...
do
    ...
    ls -la /resources/assemblies/ref-transcriptome.gtf | awk '{print $3}' | sort > /tmp/fs-output$PID
    diff /tmp/s3-output$PID /tmp/fs-output$PID  > /tmp/s3-local-file-diff$PID
